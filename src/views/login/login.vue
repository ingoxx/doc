...
.auth-fullscreen-overlay {
	...
	.glow-orb {
		...
		z-index: 1; pointer-events: none;
		&.orb-blue { top: -10%; left: 20%; background: rgba(14, 165, 233, 0.18); }
		&.orb-purple { bottom: -10%; right: 20%; background: rgba(99, 102, 241, 0.18); }
	}
	...
	.auth-card {
		...
		.security-badge-wrapper { display: flex; justify-content: center; margin-bottom: 20px; }
		.security-badge {
		...
		.card-body {
			...
			::v-deep .el-input__inner {
				...
				border: 1px solid #27272a !important; color: #f8fafc !important;
				height: 48px; line-height: 48px; font-size: 14px; border-radius: 10px;
				padding-left: 42px !important;
				&:focus { border-color: #38bdf8 !important; box-shadow: 0 0 0 3px rgba(56, 189, 248, 0.18) !important; }
				&::placeholder { color: #64748b; }
			}
			::v-deep .el-input__prefix { left: 14px; color: #64748b; i { line-height: 48px; font-size: 17px; } }
			::v-deep .el-input__suffix { right: 12px; i { line-height: 48px; font-size: 16px; color: #64748b; } }
		}
		...
		.card-footer {
			.unlock-btn {
				...
				.arrow-icon { margin-left: 8px; font-size: 16px; }
				&:hover { transform: translateY(-1px); box-shadow: 0 8px 24px rgba(14, 165, 233, 0.45); }
			}
			.action-links {
				margin-top: 16px; display: flex; align-items: center; justify-content: space-between; font-size: 13px;
				.forgot-btn { color: #94a3b8 !important; padding: 0 !important; font-size: 13px; &:hover { color: #f59e0b !important; } }
				.link-divider { color: #334155; margin: 0 6px; }
				.register-group { display: inline-flex; align-items: center; }
				.tip-text { color: #64748b; }
				.register-btn { color: #38bdf8 !important; font-weight: 600; padding: 0 2px !important; font-size: 13px; &:hover { color: #7dd3fc !important; text-decoration: underline; } }
			}
			.footer-tips { margin-top: 14px; text-align: center; color: #475569; font-size: 12px; display: flex; align-items: center; justify-content: center; gap: 4px; }
		}
	}
}

@keyframes pulse-ring { 0% { transform: scale(0.95); opacity: 0.8; } 50% { transform: scale(1.15); opacity: 0; } 100% { transform: scale(0.95); opacity: 0; } }
.shake-anim { animation: card-shake 0.5s cubic-bezier(0.36, 0.07, 0.19, 0.97) both; }
@keyframes card-shake { 10%, 90% { transform: translate3d(-1px, 0, 0); } 20%, 80% { transform: translate3d(2px, 0, 0); } 30%, 50%, 70% { transform: translate3d(-4px, 0, 0); } 40%, 60% { transform: translate3d(4px, 0, 0); } }
</style>